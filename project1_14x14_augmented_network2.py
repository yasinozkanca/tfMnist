...
def augmentImages(downSampled):
    ...
    for i in range(len(downSampled)):
        ...
        temp1Img = ndimage.interpolation.rotate(tempImg, 20,reshape=False)

        temp1Img = temp1Img.reshape([1,196])
        rotated[i, :] = temp1Img
        temp2Img = ndimage.interpolation.affine_transform(tempImg,[2,2],output_shape=((14,14)))
        temp2Img = temp2Img.reshape([1,196])

        zoomed[i, :] = temp2Img
        temp3Img = ndimage.interpolation.shift(tempImg, [8, 6])
        temp3Img = temp3Img.reshape([1, 196])
        shifted[i,:] = temp3Img
...
def incTrainSamples(trainImages,labels):
    ...
    shifted, zoomed, rotated = augmentImages(downSampled)

    trainImages = np.concatenate((downSampled, shifted))
    trainImages = np.concatenate((trainImages, zoomed))
    trainImages = np.concatenate((trainImages, rotated))
    tempLabels = labels
    labels = np.concatenate((labels, tempLabels))
    labels = np.concatenate((labels, tempLabels))
    labels = np.concatenate((labels, tempLabels))
    newBatch = (trainImages, labels)
    return newBatch
